...
export default function AdminDashboard() {
  ...
  if (authLoading || loading) return <div className="p-8 text-gray-900 dark:text-white">Loading Admin Dashboard...</div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8 min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 shadow rounded-lg">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Most Popular Policies</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
             <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Subscriptions</dt>
             <dd className="mt-1 text-3xl font-semibold text-gray-900 dark:text-white">{totalSubscriptions}</dd>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6">
          <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">Recent Subscriptions (Policy Purchases)</h3>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Policy</th>
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {subscriptions.slice(0, 15).map((s) => (
                <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{s.profiles?.email}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${s.status === 'active' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 
                        s.status === 'cancelled' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' : 
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1 border border-blue-300 dark:border-blue-600 text-sm font-medium rounded-md text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
                    >
                      View Details
                    </button>
                  </td>
              ...
              {subscriptions.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No subscriptions found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {selectedSubscription && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{selectedSubscription.profiles?.full_name || 'Unknown'}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-gray-400">Email:</span>
                    <p className="font-medium text-gray-900 dark:text-white">{selectedSubscription.profiles?.email || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{selectedSubscription.policies?.name || 'N/A'}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 dark:text-gray-400">Frequency:</span>
                    <p className="font-medium text-gray-900 dark:text-white capitalize">{selectedSubscription.frequency}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p>
                      <span className={`px-2 py-1 rounded text-xs font-medium 
                        ${selectedSubscription.status === 'active' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 
                          selectedSubscription.status === 'cancelled' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' : 
                          'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300'}`}>
              ...
              <div>
                ...
                <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p>
                        <span className={`px-2 py-1 rounded text-xs font-medium capitalize
                          ${selectedSubscription.health_status === 'excellent' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 
                            selectedSubscription.health_status === 'good' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300' : 
                            selectedSubscription.health_status === 'fair' ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300' : 
                            selectedSubscription.health_status === 'preexisting' ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' : 
                    ...
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">Smoking Status:</span>
                      <p>
                        {selectedSubscription.is_smoker !== null && selectedSubscription.is_smoker !== undefined ? (
                          selectedSubscription.is_smoker ? (
                            <span className="px-2 py-1 rounded text-xs font-medium bg-orange-100 dark:bg-orange-900/50 text-orange-800 dark:text-orange-300">Smoker</span>
                          ) : (
                            <span className="px-2 py-1 rounded text-xs font-medium bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300">Non-Smoker</span>
                          )
                        ) : 'Not provided'}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500 dark:text-gray-400">Medical Document:</span>
                      <p>
            ...
            {/* Modal Footer */}
            <div className="bg-gray-50 dark:bg-gray-700 px-6 py-4 rounded-b-xl flex justify-end">
              <button
                onClick={() => setSelectedSubscription(null)}
